...
void setup()
{
	...
	if (!SD.begin(PIN_CS))                             // инициализация SD карты с указанием номера вывода CS  
	{
		...
		return;											 // ошибка инициализации. карта не обнаружена или не подключён (неправильно подключён) адаптер карт MicroSD
	}
}
...
void loop()
{
	...
	if (time.gettime("s") == ("15"))								// если прошла 60 секунда
	{
